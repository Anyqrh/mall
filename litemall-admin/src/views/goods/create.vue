...
<!--          <editor :init="editorInit" v-model="goods.detail"/>-->
          <div  v-loading="imageLoading"
            ...
            <quill-editor ref="newEditor"  style="height: 500px" v-model="goods.detail" :options="editorOption" ></quill-editor>
            <!-- 文件上传input 将它隐藏-->
            <el-upload style="display:none"  :action="uploadPath" :headers="headers" :show-file-list="false" :before-upload="beforeUpload" :on-success='newEditorSuccess'
...
export default {
  ...
  methods: {
    ...
    newEditorSuccess(response, file, fileList){
      ...
      this.$refs.newEditor.quill.insertEmbed(this.addImgRange != null ? this.addImgRange.index : 0, 'image', response.data.url);
      // 将光标移到最后一位
